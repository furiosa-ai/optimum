...
# based on https://github.com/huggingface/optimum/blob/v1.9.1/optimum/exporters/onnx/__main__.py#L52
def export_toy_onnx(
    json_path: str,
    ...
    **kwargs_shapes,
    ...
    """
    Full-suite ONNX with no pretrined weights export.

    Args:
        > Required parameters

        json_path (`str`):
            Model config path in json format.
        output (`Union[str, Path]`):
    ...
    framework = TasksManager.determine_framework(
        json_path, subfolder=subfolder, framework=framework
    )
    ...
    model = TasksManagerToy.get_model_from_json(
        task,
        json_path,
        subfolder=subfolder,
        revision=revision,
...
class TasksManagerToy(TasksManager):
    # based on https://github.com/huggingface/optimum/blob/v1.9.1/optimum/exporters/tasks.py#L1392
    @staticmethod
    def get_model_from_json(
        ...
        **model_kwargs,
        ...
        """
        Retrieves a model (with no pretrained weights) from its name and the task to be enabled.
        ...
        if task == "auto":
            task = TasksManager.infer_task_from_model(
                json_path, subfolder=subfolder, revision=revision
            )
        ...
        if TasksManager._TASKS_TO_LIBRARY[task.replace("-with-past", "")] == "transformers":
            config = AutoConfig.from_pretrained(json_path, **kwargs)
            ...
            if (
                original_task == "automatic-speech-recognition"
        ...
        # NOTE: no pretrained weights are loaded.
